...
- [ ] **B3. Весь ростер разом + сводка**:
  ```bash
  TWELVEDATA_API_KEY=твой_ключ python smc_backtest.py --days 365 --report-dir data/backtest/reports
  ```
  Жду: три отчёта (ETHUSD и USDJPY из кэша — быстро, USDCAD докачается) и
  блок `COMBINED` со строками ETHUSD, USDJPY, USDCAD и TOTAL.

- [ ] **B4. Прислать отчёты мне** — все файлы из
