        ...
        try:
            ...
            data = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a financial analyst specializing in Forex markets.",
                    },
...
class ScrapingService:
    ...
    async def scrape_forex_calendar(self, date: datetime) -> List[ForexNewsCreate]:
        ...
        try:
            ...
            for item in news_items:
                ...
                try:
                    event_time = datetime.combine(
                        date.date(), datetime.strptime(item["time"], "%H:%M").time()
                    )
                except ValueError:
            ...
            logger.error(
                "Failed to scrape forex calendar",
                date=date,
                error=str(e),
                exc_info=True,
            )
        ...
        try:
            ...
            try:
                ...
                html = await asyncio.to_thread(
                    self.selenium_scraper.fetch_with_undetected_chromedriver, url
                )
                ...
                raise CloudflareBypassError(
                    f"All scraping methods failed: {e}, fallback: {fallback_e}"
                )
        ...
        if analysis_required:
            ...
            for group_items in grouped.values():
                if len(group_items) > 1:
                    ...
                    group_items[0]["analysis"] = self.analyzer.analyze_news(
                        group_items[0]
                    )
    ...
    def _parse_news_from_html(self, html: str) -> List[Dict[str, str]]:
        ...
        for selector in selectors:
            ...
            if rows:
                logger.info(
                    "Found rows with selector", count=len(rows), selector=selector
                )
                break
        ...
        if not rows:
            logger.warning(
                "No news rows found with any selector. Saving HTML to /tmp/forex_debug.html for inspection."
            )
            try:
    ...
    async def scrape_investing_com_calendar(
        self, date: datetime
    ) -> List[Dict[str, Any]]:
