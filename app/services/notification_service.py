...
class NotificationDeduplicationService:
    ...
    def _generate_notification_id(self, event_type: str, **kwargs) -> str:
        ...
        params_str = (
            f"{event_type}:{':'.join(f'{k}={v}' for k, v in sorted(kwargs.items()))}"
        )
    ...
    def _cleanup_old_notifications(self):
        ...
        if now - self.last_cleanup > self.cleanup_interval:
            ...
            old_notifications = [
                notification_id
                for notification_id, timestamp in self.sent_notifications.items()
            ...
            old_group_notifications = [
                notification_id
                for notification_id, timestamp in self.group_notifications.items()
                if timestamp < cutoff_time
            ]
            ...
            old_chart_timestamps = [
                target
                for target, timestamp in self.last_chart_sent.items()
                if timestamp < cutoff_time
            ]
...
class NotificationService:
    ...
    async def send_news_notification(
        ...
        try:
            ...
            if self.deduplication.is_notification_sent(
                "news_notification",
                user_id=user_id,
                ...
                logger.info(
                    "News notification already sent",
                    user_id=user_id,
                    date=target_date.date(),
                )
                return True
            ...
            if success:
                ...
                self.deduplication.mark_notification_sent(
                    "news_notification",
                    user_id=user_id,
                    date=target_date.date(),
                    impact_level=impact_level,
    ...
    async def send_group_notification(
        ...
        try:
            ...
            if self.deduplication.is_group_notification_sent(group_key):
                logger.info(
                    "Group notification already sent",
                    chat_id=chat_id,
                    date=target_date.date(),
                )
                return True
    ...
    async def _send_news_chart(
        self, target_id: int, news_items: List[Dict[str, Any]], target_date: datetime
    ) -> bool:
        """Send chart for news items."""
        try:
            # Get the first high-impact news item for chart generation
            high_impact_items = [
                item for item in news_items if item.get("impact") == "high"
            ]
    ...
    async def send_daily_digest(
        ...
        try:
            ...
            if self.deduplication.is_notification_sent(
                "daily_digest", user_id=user_id, date=target_date.date()
            ):
                logger.info(
                    "Daily digest already sent",
                    user_id=user_id,
                    date=target_date.date(),
                )
                return True
            ...
            message = await self._format_digest_message(
                news_items, target_date, user_preferences
            )
    ...
    async def _format_digest_message(
        ...
        if user_preferences:
            ...
            impact_levels = user_preferences.get(
            ...
            if preferred_currencies:
                news_items = [
                    ...
                    if item.get("currency") in preferred_currencies
                ]

            if impact_levels != ["high", "medium", "low"]:
                news_items = [
                    item for item in news_items if item.get("impact") in impact_levels
                ]
    ...
    async def send_error_notification(
        self, user_id: int, error_message: str, error_type: str = "general"
    ) -> bool:
        ...
        try:
            ...
            if self.deduplication.is_notification_sent(
                "error_notification", user_id=user_id, error_type=error_type
            ):
                logger.info(
            ...
            success = await self.telegram_service.bot_manager.send_message(
                user_id, message
            )
    ...
    async def send_system_notification(
        self, user_id: int, message: str, notification_type: str = "system"
    ) -> bool:
        ...
        try:
            ...
            success = await self.telegram_service.send_long_message(
                user_id, formatted_message
            )
        ...
        try:
            with self.deduplication._lock:
                return {
                    ...
                    "cleanup_interval_hours": self.deduplication.cleanup_interval.total_seconds()
                    / 3600,
                }
        except Exception as e:
