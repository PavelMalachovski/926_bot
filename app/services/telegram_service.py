...
class TelegramBotManager:
    ...
    async def setup_webhook(
        self, max_retries: int = 5, initial_delay: int = 10
    ) -> bool:
                        ...
                        if new_webhook.get("url") == webhook_url:
                            ...
                            if new_webhook.get("pending_update_count", 0) > 0:
                                logger.info(
                                    ...
                                    count=new_webhook.get("pending_update_count"),
                                )

                            # Check for any recent errors
                            if new_webhook.get("last_error_message"):
                                logger.warning(
                                    "⚠️ Webhook has recent error",
                                    error=new_webhook.get("last_error_message"),
                                )
    ...
    async def check_webhook_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "is_configured": webhook_info.get("url") is not None
                and webhook_info.get("url") != "",
            }
        except Exception as e:
            return {"error": str(e)}
    ...
    async def send_message(
        ...
        try:
            ...
            logger.error(
                "Failed to send message", chat_id=chat_id, error=str(e), exc_info=True
            )
            raise TelegramError(f"Failed to send message: {e}")

    async def send_photo(
        self, chat_id: int, photo: bytes, caption: Optional[str] = None
    ) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                response = await client.post(
                    f"{self.base_url}/sendPhoto", data=data, files=files, timeout=30.0
                )
            ...
            logger.error(
                "Failed to send photo", chat_id=chat_id, error=str(e), exc_info=True
            )
    ...
    async def send_document(
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    logger.info(
                        "Document sent successfully", chat_id=chat_id, filename=filename
                    )
                    return True
                else:
                    logger.error(
    ...
    async def send_formatted_message(self, chat_id: int, message: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to send formatted message", chat_id=chat_id, error=str(e)
            )
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.base_url}/getWebhookInfo", timeout=10.0
                )
    ...
    async def set_webhook(self, url: str, secret_token: Optional[str] = None) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                response = await client.post(
                    f"{self.base_url}/setWebhook", json=payload, timeout=10.0
                )
    ...
    async def delete_webhook(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.base_url}/deleteWebhook", timeout=10.0
                )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    if result.get("ok"):
                        logger.info(
                            "Callback query answered successfully",
                            callback_query_id=callback_query_id,
                        )
                        return True
                    else:
    ...
    def _ping_self(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    "Ping failed", url=ping_url, status_code=response.status_code
                )
        except Exception as e:
...
class TelegramService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if connection_test.get("success"):
                ...
                logger.error(
                    "Telegram bot connection failed", error=connection_test.get("error")
                )
            ...
            logger.error(
                "Failed to initialize Telegram service", error=str(e), exc_info=True
            )
            raise
    ...
    async def send_long_message(
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                success = await self.bot_manager.send_message(
                    chat_id, chunk, parse_mode
                )
                if not success:
                    logger.error(
            ...
            logger.info(
                "Long message sent successfully", chat_id=chat_id, chunks=len(chunks)
            )
            return True

        except Exception as e:
            logger.error(
                "Failed to send long message",
                chat_id=chat_id,
                error=str(e),
                exc_info=True,
            )
    ...
    async def format_news_message(
        ...
        for (currency, time), items in sorted(grouped.items()):
            ...
            for idx, item in enumerate(items):
                ...
                actual = (
                    str(item["actual"]).replace("\\", "") if item["actual"] else "N/A"
                )
                forecast = (
                    str(item["forecast"]).replace("\\", "")
                    if item["forecast"]
                    else "N/A"
                )
                previous = (
                    str(item["previous"]).replace("\\", "")
                    if item["previous"]
                    else "N/A"
                )
                analysis = (
                ...
                if (
                    analysis_required
                    and not item.get("group_analysis", False)
                    and analysis
                ):
    ...
    async def process_webhook_update(self, update_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to process webhook update", error=str(e), exc_info=True
            )
            return False
    ...
    async def _handle_settings_command(self, message):
        ...
        try:
            # Get user settings
            user_prefs = await self.db_service.get_user_preferences(
            ...
            if user_prefs:
                ...
                await self.bot_manager.send_message(
                    message.chat.id,
                    "❌ Settings not available. Please try again later.",
                )
        except Exception as e:
            logger.error("Failed to handle settings command", error=str(e))
            await self.bot_manager.send_message(
                message.chat.id, "❌ Error retrieving settings."
            )
