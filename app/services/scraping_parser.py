                ...
                if ":" in time_str:
                    if "am" in time_str.lower() or "pm" in time_str.lower():
                        ...
                        time_obj = datetime.strptime(
                            time_str.lower().replace("am", " AM").replace("pm", " PM"),
                            "%I:%M %p",
                        )
                    else:
    ...
    def extract_news_data(self, row) -> Dict[str, str]:
        ...
        if impact_element:
            ...
            logger.debug(
                "Classes",
                classes=classes,
                mapping_keys=list(self.IMPACT_CLASS_MAP.keys()),
            )
                ...
                for c in classes:
                    ...
                    if c_eq in self.IMPACT_CLASS_MAP:
                        logger.debug(
                            ...
                            class_name=c,
                            as_class=c_eq,
                            level=self.IMPACT_CLASS_MAP[c_eq],
                        )
                    ...
                    if c_dash in self.IMPACT_CLASS_MAP:
                        logger.debug(
                            ...
                            class_name=c,
                            as_class=c_dash,
                            level=self.IMPACT_CLASS_MAP[c_dash],
                        )
            ...
            if impact == "unknown":
                if (len(classes) == 1 and classes[0] == "icon") or not classes:
                    if time_24.lower() in ["tentative", "all day"] or time.lower() in [
                        "tentative",
                        "all day",
                    ]:
                        impact = "tentative"
