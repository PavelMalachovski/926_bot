...
class ChartService:
    ...
    def __init__(
        ...
        enable_alpha_vantage: Optional[bool] = None,
        enable_alternative_symbols: Optional[bool] = None,
    ):
        self.cache_dir = cache_dir or os.path.join(
            tempfile.gettempdir(), "forex_charts"
        )
        ...
        self.yf_min_request_interval_sec = getattr(
            settings.chart, "yf_min_request_interval_sec", 1
        )
        self._last_yf_request_time = 0

        # Display timezone
        self.display_timezone = pytz.timezone(
            getattr(settings.chart, "display_timezone", "Europe/Prague")
        )
    ...
    def _get_symbol_from_currencies(
    ...
    async def _fetch_price_data(self, request: ChartRequest) -> Optional[ChartData]:
        ...
        try:
            ...
            symbol = self._get_symbol_from_currencies(
                request.currency[:3], request.currency[3:]
            )
    ...
    async def _fetch_alpha_vantage_data(
        ...
        try:
            ...
            if ohlc_data:
                logger.info(
                    "Successfully fetched Alpha Vantage data", rows=len(ohlc_data)
                )
                return ChartData(
                    symbol=symbol,
                    currency=request.currency,
    ...
    async def _generate_chart_image(
        self, chart_data: ChartData, request: ChartRequest
    ) -> bytes:
    ...
    async def create_event_chart(
        ...
        try:
            ...
            if response.success:
                ...
                logger.error(
                    "Failed to create event chart", error=response.error_message
                )
                return None
    ...
    async def get_chart_for_currency(
        ...
        try:
            ...
            if response.success:
                ...
                logger.error(
                    "Failed to get currency chart", error=response.error_message
                )
                return None
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            av_key_configured = (
                bool(self.alpha_vantage_api_key) if self.enable_alpha_vantage else True
            )

            return {
                "status": (
                    "healthy"
                    if all([cache_accessible, charts_accessible, av_key_configured])
                    else "unhealthy"
                ),
                "cache_dir": self.cache_dir,
