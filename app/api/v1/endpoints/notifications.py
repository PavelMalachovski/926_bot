...
async def mark_notification_failed(
    ...
    try:
        success = await notification_service.mark_notification_failed(
            db, notification_id, error_message
        )
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Notification not found"
...
async def create_digest_notification(
    ...
    try:
        notification = await notification_service.create_digest_notification(
            db, user_id, digest_time
        )
        return NotificationResponse.model_validate(notification)
    except ValidationError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
