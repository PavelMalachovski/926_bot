...
async def get_upcoming_events(
    hours: int = Query(
        24, ge=1, le=168, description="Hours ahead to look for upcoming events"
    ),
    db: AsyncSession = Depends(get_database),
    forex_service: ForexService = Depends(get_forex_service),
):
...
async def get_forex_news(
    ...
    try:
        ...
        if not news:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Forex news not found"
            )
        return ForexNewsResponse.model_validate(news)
    except DatabaseError as e:
        raise HTTPException(
...
async def get_forex_news_list(
    ...
    try:
        ...
        news_list = await forex_service.get_all(
...
async def get_forex_news_by_date_range(
    ...
    try:
        news_list = await forex_service.get_forex_news_by_date_range(
...
@router.post(
    "/bulk/",
    response_model=List[ForexNewsResponse],
