        ...
        return JSONResponse(
            content={
                ...
                "connection_pool_size": (
                    cache_service.connection_pool.size
                    if cache_service.connection_pool
                    else 0
                ),
            }
        )
...
async def invalidate_cache_pattern(
    ...
    try:
        ...
        logger.error(
            "Failed to invalidate cache pattern", pattern=pattern, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_cache_hit_ratio():
    ...
    try:
        ...
        return JSONResponse(
            content={"hit_ratio": hit_ratio, "percentage": round(hit_ratio * 100, 2)}
        )
    except Exception as e:
...
async def check_rate_limit(
    ...
    try:
        ...
        status_code = (
            status.HTTP_200_OK if is_allowed else status.HTTP_429_TOO_MANY_REQUESTS
        )
...
async def get_session(session_id: str):
    ...
    try:
        ...
        return JSONResponse(content={"session_id": session_id, "data": session_data})
    except HTTPException:
        raise
    except Exception as e:
